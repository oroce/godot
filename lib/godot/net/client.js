...
var Client = module.exports = function Client(options) {
  ...
  if (!options || !options.type
    ...
    throw new Error('Cannot create client without type: udp, tcp, unix, mqtt');
  }
...
Client.prototype.write = function (data) {
  ...
  if (this.type === 'tcp' || this.type === 'unix') {
    ...
    this.socket.publish(data.service || 'godot', message, extend({
      qos: 1
    }, this.mqttMsgOptions || {}));
  }
};
...
Client.prototype.connect = function (port, host, callback) {
  ...
  function reconnect(err) {
    if (self.type === 'mqtt') {
      // mqtt has its own reconnect mechanism
      return;
    }
  ...
  function connect() {
    ...
    else if (self.type === 'mqtt') {
      self.socket = mqtt.connect(extend({
        port: this.port,
        host: this.host
      }, this.mqttOptions));
    }
