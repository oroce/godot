...
var Server = module.exports = function Server(options) {
  ...
  if (!~['json', 'protobuf'].indexOf(codec)) {
    throw new Error('options.codec should be one of: json, protobuf or null');
  }
...
Server.prototype._onTcpSocket = function (socket) {
  var address = socket.remoteAddress,
      ...
      id = address + ':' + port,
      parser = this.codec === 'json' ? jsonStream() : protobufStream({
        handshake: false
      }),
      self = this,
      reactors;
  ...
  var e = parser.emit;
  parser.emit = function(ev) {
    console.log('[%s]', ev);
    return e.apply(parser, [].slice.call(arguments));
  }
  //
  // Remark: Too much churn here to emit?
  //
  ...
  socket.on('data', function(b) {
    //console.log('data is flowing in:' + b,   b);
  })
  parser.on('msg', function(msg) {
    msg.events.forEach(function(event) {
      for(var i = 0; i < reactors.length; i++) {
        console.dir(event);
        //reactors[i].socket.write(event.toJSON());
      }
    });
  });
  parser.on('unknown', function(msg) {
    console.log('unknown', arguments);
  });
  parser.on('data', function (event) {
    //console.log('event', event, '' + event, protobuffer.Msg.decode(event));
    for(var i = 0; i < reactors.length; i++) {
