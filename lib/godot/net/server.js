...
var Server = module.exports = function Server(options) {
  if (!options || !options.type
      || !~['tcp', 'udp', 'unix', 'mqtt'].indexOf(options.type)) {
    throw new Error('Cannot create server without type: udp, tcp, unix, mqtt');
  }
  ...
  this.multiplex = typeof options.multiplex !== 'undefined'
    ? options.multiplex
    : true;
  this.moscaOptions = options.moscaOptions;
...
Server.prototype.listen = function (port, host, callback) {
  ...
  if (this.type === 'udp') {
    ...
    this.server = new (mosca.Server)(extend({
      port: this.port,
    }, this.moscaOptions || {}));
    this.server.published = this._onMqttPublished.bind(this);
    this.server.once('error', respond);
    this.server.once('ready', respond);
  }
};
...
Server.prototype.close = function (callback) {
  return (['tcp', 'unix', 'mqtt'].indexOf(this.type) !== -1)
...
Server.prototype._onMqttPublished = function (packet, client, cb) {
  if (!client) {
    // no client yet, it can be connect, disco, etc
    return cb();
  }
